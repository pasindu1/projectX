...
const Dashboard = () => {
  ...
  const onCellClicked = async (clickedRow, index) => {
    ...
    console.log("index", index, rowItems);
    const rowData = rowItems.find((rowItem) => rowItem.email === clickedRow.email);
    ...
    let config = {
      ...
      data: JSON.stringify({ email, emailSubject, emailContent }),
    };
  ...
  const generateEmail = async (row) => {
    ...
    let config = {
      ...
      data: JSON.stringify({ email, clientName: name }),
    };
  ...
  return (
    <>
      <main className="py-8">
        <section className="flex flex-col items-left justify-left pt-1 pb-1 ml-6">
          <h2 className="text-black text-lg pb-4">Upload excel file here</h2>
        </section>
        <FileUploader
